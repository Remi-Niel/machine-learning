...
def determineOptimalThreshold(groundTmean, groundFmean):
	...
	for t in np.linspace(0,1,101):
		TP = math.max(0,sum(1 for x in groundTmean if x >= t))
		FN = math.max(0,len(groundTmean) - TP)
		FP = math.max(0,sum(1 for x in groundFmean if x >= t))
		TN = math.max(0,len(groundFmean) - FP)
